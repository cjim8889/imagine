...
sys.path.append(os.path.join(current_file_directory, "CodeFormer"))
...
pretrain_model_url = {
    ...
    # 'realesrgan': 'https://drive.google.com/u/1/uc?id=10h8YXKKOQ61ANnwLjjHqXJdn4SbBuUku&export=download'
}
...
def setup_codeformer():
    ...
    ckpt_path = os.path.join(parent_directory, "model_codeformer", "codeformer.pth")
    checkpoint = torch.load(ckpt_path)["params_ema"]
    codeformer_net.load_state_dict(checkpoint)
    codeformer_net.eval().to(device)
    return upsampler, codeformer_net

def codeformer_inference(upsampler, codeformer_net, image, background_enhance, face_upsample, upscale, codeformer_fidelity):
